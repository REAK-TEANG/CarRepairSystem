...
export default function UnauthorizedPage() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-6 bg-app-bg text-app-text font-sans transition-colors duration-200">
      <div className="max-w-md w-full bg-app-card border border-app-border p-8 rounded-xl shadow-card text-center transition-colors duration-200">
        <div className="w-14 h-14 bg-amber-500/10 border border-amber-500/20 text-amber-500 rounded-xl flex items-center justify-center mx-auto mb-4">
          <ShieldWarning size={32} weight="duotone" />
        </div>

        <h1 className="text-xl font-bold text-app-text">Access Restricted</h1>
        <p className="text-xs text-app-muted mt-2 leading-relaxed">
          Your current active role <span className="font-semibold text-app-accent font-mono">({user?.role})</span> does not have sufficient permissions to view this system module.
        </p>
